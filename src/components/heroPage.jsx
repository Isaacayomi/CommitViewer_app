...
const Home = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            console.error('Error fetching GitHub repositores: ', error);
        }
    };
        ...
        <div>
            ...
            <div className='mx-[2rem]'>
                <div className="xl:flex xl:justify-center xl:items-center xl:gap-[16px]">
                    <div className='flex flex-row-reverse justify-end items-center gap-[0.3rem] py-[0.94rem] mb-[1.5rem] max-w-[24.75rem] w-[100%] mx-[auto] rounded-[0.5rem] bg-[#DFE4EA] xl:max-w-[694px] xl:w-[100%] xl:mr-[0]'>
                        <input type='text' onChange={handleInput} placeholder='Eg. facebook/react' className='font-inter text-[1.25rem] font-[400] leading-[1.75rem] tracking-[-0.03125rem] text-neutral w-[10.6875rem] bg-transparent outline-none  ' />
            ...
            <div className='xl:flex xl:items-center xl:justify-center xl:m-[0] xl:gap-[32px] xl:mb-[100px]'>
                ...
                {data.slice(0, 4).map((repo) => (
                    <button key={repo.id} className='max-w-[8.75rem] w-[100%] mx-[auto] mb-[1rem] py-[0.5rem] px-[1rem] bg-[#29335C] rounded-[0.5rem] block text-white text-center text-[1rem] font-[600] leading-normal tracking-[-0.025rem] xl:m-[0]' onClick={handleClick}>
                        <Link to={`/ViewCommit/${repo.name}`}>
                            {`${repo.name}`}
                        </Link>
                    </button>
